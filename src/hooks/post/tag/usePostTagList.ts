...
import { getPostTags } from '@/services';
...
const usePostTagList = ({
  ...
  const {
    ...
    handleSearch,
    handleTableChange
  } = useQueryList<IPostTagItem, IBasePageResponse<IPostTagItem[]>>(getPostTags);

  const columns: TableColumnType<IPostTagItem>[] = [
    {
      title: '标签名称',
      dataIndex: 'name',
      resizable: true,
  ...
  const getOperations = (record: IPostTagItem): IOperationButtonProps[] => {
    return [
